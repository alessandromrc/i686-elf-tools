...
function updateNupgrade {

    echoColor "Updating and Upgrading the system"

    sudo apt update && sudo apt upgrade
    echoColor "System updated and upgraded"
}
...
function installPackages {
    ...
    sudo -E apt-get -qq install git \
        autoconf automake autopoint bash bison bzip2 flex gettext g++ gperf intltool libffi-dev libgdk-pixbuf2.0-dev \
        libtool libltdl-dev libssl-dev libxml-parser-perl make \
        openssl p7zip-full patch perl pkg-config python ruby scons \
        sed unzip wget xz-utils libtool-bin texinfo g++-multilib lzip -y
    echoColor "Packages installed"
...
function downloadSources {
    ...
    if [[ $BUILD_GCC == true || $ALL_PRODUCTS == true ]]; then
        downloadAndExtract "gcc" $GCC_VERSION "https://ftp.gnu.org/gnu/gcc/gcc-$GCC_VERSION/gcc-$GCC_VERSION.tar.gz"
...
function compileBinutils {
    if [[ $BUILD_BINUTILS == true || $ALL_PRODUCTS == true ]]; then
        ...
        configureArgs="--target=${BUILD_TARGET} --with-sysroot --disable-nls --disable-ewarning --prefix=$BUILD_DIR/$1/output"
...
function compileGDB {
    if [[ $BUILD_GDB == true || $ALL_PRODUCTS == true ]]; then
        ...
        configureArgs="--target=${BUILD_TARGET} --disable-nls --disable-ewarning --prefix=$BUILD_DIR/$1/output"
...
function finalize {
    if [[ $ZIP == true || $ALL_PRODUCTS == true ]]; then
        ...
        echo -e "\e[5mZipped everything to $BUILD_DIR/${BUILD_TARGET}-tools-[windows | linux].zip\e[39m"
    else
